...
int load_table(struct image *img){
  long ii,j;
  img->btable.firstelem=1;
  img->btable.firstrow=1;
  strcpy(img->btable.strnull, " ");
...
int print_table(struct image *img){
   long j;
   for(j=0; j<img->btable.nrows; j++){
		printf("%s %ld %lf \n", img->btable.row[j].str,img->btable.row[j].inum,img->btable.row[j].rnum);
  }
...
int add_table_struct(struct image *img, struct trow *row){
  ...
  fprintf(stderr,"add_table() defined columns metadata\n");
    ...
    img->btable.planet[1] =  row[1].str;
    img->btable.planet[2] = row[2].str;
    img->btable.planet[3] = row[3].str;
    img->btable.planet[4] = row[4].str;
    img->btable.planet[5] = row[5].str;
    img->btable.diameter[0] = row[0].inum;
    img->btable.diameter[1] = row[1].inum;
    img->btable.diameter[2] = row[2].inum;
    img->btable.diameter[3] = row[3].inum;
    img->btable.diameter[4] = row[4].inum;
    img->btable.diameter[5] = row[5].inum;
    img->btable.density[0]  = row[0].rnum;
    img->btable.density[1]  = row[1].rnum;
    img->btable.density[2]  = row[2].rnum;
